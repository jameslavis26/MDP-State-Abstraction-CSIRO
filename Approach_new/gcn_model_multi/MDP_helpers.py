...
def multiclass_recall_score(y_true, y_pred, average='macro'):
    ...
    if average not in ('macro', 'micro', 'weighted', 'inv_weighted'):
        raise ValueError("Invalid 'average' parameter. Use 'macro', 'micro', or 'weighted' or 'inv_weighted'.")
    ...
    elif average == 'inv_weighted':
        class_counts = [torch.sum(y_true == class_label).item() for class_label in range(num_classes)]
        total_samples = len(y_true)
        weights = [total_samples/count for count in class_counts]
        weights = [weight/sum(weights) for weight in weights]
        inv_weighted_recall = sum([recall_per_class[i] * weights[i] for i in range(num_classes)])
        return inv_weighted_recall
