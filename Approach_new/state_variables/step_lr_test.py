...
final_lr = 0.0001
steps = 5
step_lr_factor = (final_lr/lr)**(1/steps)

lr_shedule = True
step_lr_stepsize=2000
# step_lr_factor=0.1
...
for mdp_configs in training_configs["MDP sizes"]:
    ...
    filename = f'Reserve_MDP_{N_states}_{K}_step_lr'
    ...
    for trial_num, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        if lr_shedule:
            lr_sheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_lr_stepsize, gamma=step_lr_factor)
